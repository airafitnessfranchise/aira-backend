...
wss.on('connection', (ws, req) => {
  ...
  ws.on('close', () => { if (registeredLocationId) { tabletConnections.delete(registeredLocationId); console.log('[WS] Tablet disconnected: ' + registeredLocationId); } });
...
app.post('/upload/recording', upload.single('audio_file'), async (req, res) => {
  ...
  if (recording) {
    await db.updateRecording(recording.recording_id, { audio_file_url: file.path, duration_seconds: parseInt(duration_seconds) || 0, processing_status: 'uploaded', contact_name: contact_name || recording.contact_name || 'Walk-in' });
  } else {
    recording = await db.createRecording({ appointment_id: appointment_id || ('manual-' + Date.now()), location_id: location_id || 'unknown', duration_seconds: parseInt(duration_seconds) || 0, audio_file_url: file.path, contact_name: contact_name || 'Walk-in' });
  }
...
async function processRecording(recording_id, audioFilePath, location_id, appointment_id) {
  const location = byLocationId[location_id] || { location_id: location_id || 'unknown', franchise_name: 'Walk-in / Unknown Location', franchisee_name: 'Franchisee', franchisee_email: process.env.MIKE_EMAIL || 'mikebell@airafitness.com' };
  try {
    console.log('[Pipeline] Uploading to R2: ' + recording_id);
    const r2Key = await uploadToR2(audioFilePath, recording_id);
    if (r2Key) { await db.updateRecording(recording_id, { r2_key: r2Key }); console.log('[Pipeline] R2 key stored: ' + r2Key); }
    console.log('[Pipeline] Transcribing ' + recording_id);
    await db.updateRecording(recording_id, { processing_status: 'transcribing' });
    const transcript = await transcribeAudio(audioFilePath);
    await db.updateRecording(recording_id, { transcript: transcript, processing_status: 'transcribed' });
    ...
    const savedScorecard = await db.createScorecard({ recording_id: recording_id, scorecard: scorecard });
    ...
    if (location && recording) {
      ...
      if (savedScorecard.total_score < threshold) console.log('[Pipeline] Score ' + savedScorecard.total_score + ' below threshold — flagged');
    }
...
app.get('/scorecard/:id', async (req, res) => {
  try {
    ...
    const loc = byLocationId[r.location_id] || {};
    const name = r.contact_name || r.appointment_id;
    const date = new Date(r.recorded_at).toLocaleDateString('en-US', { timeZone: 'America/Chicago', month: 'long', day: 'numeric', year: 'numeric' });
...
app.get('/playback/:recording_id', async (req, res) => {
  ...
  res.send('<!DOCTYPE html><html><head><title>Playback</title><style>body{background:#0a0a0a;color:#f0f0f0;font-family:Arial;padding:32px}h2{color:#c8f060}</style></head><body><h2>Recording: ' + name + '</h2><p style="color:#666;margin-bottom:16px">' + new Date(rec.recorded_at).toLocaleString('en-US', { timeZone: 'America/Chicago' }) + '</p><audio controls style="width:100%;margin-bottom:24px"><source src="/audio/' + path.basename(rec.audio_file_url) + '"></audio>' + (rec.transcript ? '<h3 style="color:#888;margin-bottom:8px">Transcript</h3><p style="line-height:1.7;color:#ccc">' + rec.transcript + '</p>' : '<p style="color:#555">No transcript yet</p>') + '<p style="margin-top:16px"><a href="/admin" style="color:#c8f060">Back to admin</a></p></body></html>');
});
...
app.get('/status', async (req, res) => {
  const recordings = await db.getAllRecordings();
  const scorecards = await db.getAllScorecards();
  res.json({ status: 'ok', tablets_connected: tabletConnections.size, tablets: Array.from(tabletConnections.keys()), recordings: recordings.length, scorecards: scorecards.length, uptime_seconds: Math.round(process.uptime()) });
});
...
app.post('/test/trigger', async (req, res) => {
  ...
  const recording = await db.createRecording({ appointment_id: apptId, location_id: location_id });
