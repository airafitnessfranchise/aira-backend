...
// Auth: api_key passed as URL query parameter (confirmed working)
...
async function fetchGymMasterKPI(companyId, startDate, endDate) {
  ...
  if (!apiKey) {
    console.warn('[GymMaster] GYMMASTER_STAFF_API_KEY not set');
    return null;
  }

  // GymMaster requires api_key as a URL query parameter
  const url = GM_BASE + '/api/v2/report/kpi/categories?api_key=' + apiKey;
...
async function getLocationMetrics(companyId, startDate, endDate) {
  try {
    ...
    const totalSalesField = salesMade.find(function(f) {
      return f.money && f.money.name && f.money.name.toLowerCase().includes('total sales');
    });
    ...
    const newField = memberActivity.find(function(f) {
      return f.memberships && f.memberships.name && f.memberships.name.toLowerCase().includes('new');
    });
    const newMembers = newField ? newField.memberships.value : 0;

    const currentField = memberStats.find(function(f) {
      return f.members && f.members.name && f.members.name.toLowerCase().includes('current');
    });
