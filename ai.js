...
async function scoreTranscript(transcript) {
  ...
  let response;
    try {
          ...
          console.error(`[AI] Claude API error status: ${err.response?.status}`);
          console.error(`[AI] Claude API error body: ${JSON.stringify(err.response?.data)}`);
          console.error(`[AI] ANTHROPIC_API_KEY set: ${!!process.env.ANTHROPIC_API_KEY}`);
          console.error(`[AI] Key prefix: ${process.env.ANTHROPIC_API_KEY?.substring(0, 20)}`);
          throw err;
    }
